...
/**
 *  由于SDK内部对BubbleViewModel做Cache用于显示时复用，复用前调用BubbleViewModelReuseBlock以供开发者做处理
 */
typedef void (^BubbleViewModelPrepare4UseBlock) (YWBaseBubbleViewModel * viewModel);

/**
 *  由于SDK内部对BubbleView做Cache用于显示时复用，复用前调用BubbleViewReuseBlock以供开发者做处理
 */
